...
EN: Search for recent mentions of your @handle via LocalBridge, then send
    a Feishu (Lark) webhook notification for each new mention since the
    last run. Only new mentions are reported.
    Stores the latest seen tweet ID in a local state file.
    Uses the browser extension as proxy — no OAuth token needed.
中文：通过 LocalBridge 搜索 @你的账号 的最新提及，将新提及通过飞书 Webhook 推送通知。
     每次只上报新提及，状态保存在本地文件中。
